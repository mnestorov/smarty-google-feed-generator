...
class Smarty_Gfg_API {
    ...
    private function make_api_request($endpoint, $method = 'GET', $query_params = []) {
        ...
        if (is_wp_error($response)) {
            ...
            _gfg_write_logs('License API request failed: ' . $response->get_error_message());
            return false; 
        }
    ...
    public function validate_license($license_key) {
        $response = $this->make_api_request('', 'GET', ['license_key' => $license_key]);

        if (isset($response['status']) && $response['status'] === 'active') {
            return true;
        }

        // Log the response for debugging if validation fails
        _gfg_write_logs('License validation response: ' . print_r($response, true));
        return false;
    }
}